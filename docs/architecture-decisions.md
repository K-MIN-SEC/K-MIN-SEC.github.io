...
프로필 소속은 없음·학교·기업·기타의 선택값과 소속명·학과/부서로 관리한다. 학교를 기본 전제로 삼지 않으며 선택한 유형에 맞는 양식과 공개 표시를 제공한다. 학교를 선택한 회원만 입학·졸업 예정·졸업 연도를 선택 입력할 수 있고, 실제 졸업 연도를 우선해 기수별 포트폴리오 검색에 사용한다. 기존 교육 데이터는 0010·0011 migration에서 보존한다.
