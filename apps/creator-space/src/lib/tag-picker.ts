document.querySelectorAll<HTMLElement>('[data-tag-picker]').forEach(root=>{
  ...
  let options=[...root.querySelectorAll<HTMLButtonElement>('[data-tag]')];
  ...
  const setSuggestions=(suggestions:string[])=>{const box=root.querySelector<HTMLElement>('.tag-options')!;options=suggestions.map(value=>{const b=document.createElement('button');b.type='button';b.className='tag-option';b.dataset.tag=value;b.textContent=value;b.onclick=()=>toggle(value);return b;});box.replaceChildren(...options);render();};
  root.addEventListener('tag-suggestions',()=>{if(root.dataset.suggestions)setSuggestions(JSON.parse(root.dataset.suggestions));});
  options.forEach(b=>b.onclick=()=>toggle(b.dataset.tag!));
  if(root.dataset.suggestions)setSuggestions(JSON.parse(root.dataset.suggestions));
