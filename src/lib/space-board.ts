...
if (board && !board.dataset.ready) {
  ...
  async function loadBasic() {
    ...
    for(const post of posts){
      ...
      const like=button(`${liked?'♥':'♡'} 좋아요 ${postLikes.length}`,async()=>{await ensureCommunityUser();const result=await supabase!.rpc('toggle_space_like',{p_post_id:post.id});if(result.error)throw result.error;await load();},'like-button');like.setAttribute('aria-pressed',String(liked));article.append(like);
  ...
  async function load() {
    ...
    for(const summary of summaries) {
      ...
      const like=button(`${liked?'♥':'♡'} 좋아요 ${likes.data?.length??0}`,async()=>{await ensureCommunityUser();const result=await supabase!.rpc('toggle_space_like',{p_post_id:post.id});if(result.error)throw result.error;await load();},'like-button');like.setAttribute('aria-pressed',String(Boolean(liked)));article.append(like);
